    ...
    while iters < num_iterations:
        for train_batch in train_loader:
            with torch.amp.autocast(device_type='cuda', enabled=amp_enabled):
                ...
                fake_batch = gen(n_samples=train_batch.size(0))
                ...
                # discriminator on it.
                ###################################################################
                eps = torch.rand(train_batch.size(0), 1, 1, 1, device=train_batch.device)
                interp = eps * train_batch + (1.0 - eps) * fake_batch.detach()
            ...
            if iters % log_period == 0 and iters != 0:
                with torch.no_grad():
                    with torch.amp.autocast(device_type='cuda', enabled=amp_enabled):
                        ...
                        generated_samples = generated_samples.clamp(0.0, 1.0)
                        ##################################################################
                        #                          END OF YOUR CODE                      #
                        ##################################################################
                    save_image(
