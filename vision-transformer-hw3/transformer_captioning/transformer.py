...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        if attn_mask is not None:
            ...
            if attn_mask.dim() == 2:
                additive_mask = (1.0 - attn_mask).unsqueeze(0).to(dot_product.dtype) * (-1e9)
            else:
                additive_mask = (1.0 - attn_mask).to(dot_product.dtype) * (-1e9)
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        Dh = D // H
        query = self.query_proj(query).view(N, S, H, Dh).transpose(1, 2)
        key = self.key_proj(key).view(N, T, H, Dh).transpose(1, 2)
        value = self.value_proj(value).view(N, T, H, Dh).transpose(1, 2)
        ...
        dot_product = torch.matmul(query, key.transpose(2, 3)) / math.sqrt(Dh)
        ...
        if attn_mask is not None:
            ...
            # Think about what inputs make softmax 0.
            if attn_mask.dim() == 2:
                additive_mask = (1.0 - attn_mask).view(1, 1, S, T).to(dot_product.dtype) * (-1e9)
            else:
                additive_mask = (1.0 - attn_mask).to(dot_product.dtype) * (-1e9)
            dot_product += additive_mask
        ...
        attn = F.softmax(dot_product, dim=-1)
        attn = self.dropout(attn)
        y = torch.matmul(attn, value)  # (N, H, S, Dh)
...
class SwiGLUFeedForward(nn.Module):
    def __init__(self, input_dim, dim_feedforward=2048, dropout=0.1):
        super().__init__()
        self.wi = nn.Linear(input_dim, 2 * dim_feedforward)  # gate + value
        self.wo = nn.Linear(dim_feedforward, input_dim)
        self.act = nn.SiLU()
        self.dropout = nn.Dropout(dropout)
        self.norm = nn.LayerNorm(input_dim)
    ...
    def forward(self, seq):
        ...
        x = self.act(gate) * value       # SwiGLU
        x = self.dropout(x)
        x = self.wo(x)
        x = self.dropout(x)
        return self.norm(seq + x)

class SinGLUFeedForward(nn.Module):
    def __init__(self, input_dim, dim_feedforward=2048, dropout=0.1):
        super().__init__()
        self.wi = nn.Linear(input_dim, 2 * dim_feedforward)  # gate + value
        self.wo = nn.Linear(dim_feedforward, input_dim)
    ...
    def forward(self, seq):
        x = self.wi(seq)
        gate, value = x.chunk(2, dim=-1)
        x = torch.sin(gate) * value     # SinGLU: sin(gate) ⊙ value
        x = self.dropout(x)
        x = self.wo(x)
        x = self.dropout(x)
        return self.norm(seq + x)
...
class DecoderLayer(nn.Module):
    def __init__(self, input_dim, num_heads, dim_feedforward=2048, dropout=0.1 ):
        ...
        self.feedforward_block = SwiGLUFeedForward(input_dim, dim_feedforward, dropout)
        # self.feedforward_block = SinGLUFeedForward(input_dim, dim_feedforward, dropout)
